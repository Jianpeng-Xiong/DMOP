...
%--------------------------------------------------------------------------
 ...
 % alfa and belta are parameters that control the overall shape of the POF
        function PopObj = CalObj(obj, PopDec)
            ...
            sigma=mod(floor(obj.Global.gen/(obj.tauT*rou)),3);
            W=floor(6*(sin(0.5*pi*(obj.Global.t-1)).^sigma));
            m=obj.Global.M;
            for i = m:obj.Global.D
                 temp = temp + (PopDec(:,i)+sigma - G).^2;
            end
            g=temp;
            PopObj(:,1) =((1+g).*(PopDec(:,1)+A*sin(W*pi*PopDec(:,1))))';
            PopObj(:,2) =((1+g).*(1-PopDec(:,1)+A*sin(W*pi*PopDec(:,1))))';
        end
        ...
        function P = PF(obj,~)
            ...
            W=floor(6*(sin(0.5*pi*(obj.Global.t-1)).^sigma));
            P(:,1) = (0:1/(N-1):1)';
            f1=(1+g).*(P(:,1)+A*sin(W*pi*P(:,1)));
            f2=(1+g).*(1-P(:,1)+A*sin(W*pi*P(:,1)));
            P(:,2) =(1+gt).*(1+2*A*sin(W*pi*((f1-f2)/(2*(1+gt))+1/2)));
        end
    end
end
