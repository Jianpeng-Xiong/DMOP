...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj, PopDec)
           ...
           a=2.25+2*cos(2*pi*obj.Global.t);
           tmp=G*sin(4*pi*PopDec(:,1))/(1+abs(G));
            g=1+sum((PopDec(:,2:end)-repmat(tmp,1,obj.Global.D-1)).^2,2);
            PopObj(:,1) =(g.*(PopDec(:,1)+0.1*sin(3*pi*PopDec(:,1))))+4*obj.Global.t;
            PopObj(:,2) =(g.*(1-PopDec(:,1)+0.1*sin(3*pi*PopDec(:,1))))+4*obj.Global.t;
        end
...
%             P = obj.load();
             ...
             x=linspace(0,1,N);
             a=2.25+2*cos(2*pi*t);
             P(:,1)=x+0.1*sin(3*pi*x)+4*t;
             P(:,2)=(1-x+0.1*sin(3*pi*x)).^a+4*t;
        end
    end
end