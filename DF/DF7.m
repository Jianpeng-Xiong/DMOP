...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function obj = DF7()
            ...
            obj.Global.lower    = [1 zeros(1,obj.Global.D-1)];
            obj.Global.upper    = [4 ones(1,obj.Global.D-1)];
            obj.Global.encoding = 'real';
        end
        %% Calculate objective values
        function PopObj = CalObj(obj, PopDec)
           a=5*cos(0.5*pi*obj.Global.t);
           tmp=1./(1+exp(a*(PopDec(:,1)-2.5)));
           g=1+sum((PopDec(:,2:end)-repmat(tmp,1,obj.Global.D-1)).^2,2);
            PopObj(:,1) =(g.*(1+obj.Global.t)./PopDec(:,1))+2*obj.Global.t;
            PopObj(:,2) =(g.*PopDec(:,1)/(1+obj.Global.t))+2*obj.Global.t;
        end
