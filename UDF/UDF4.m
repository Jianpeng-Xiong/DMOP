...
% The code cannot run directly on your computer and is for reference only.
    ...
    methods
        ...
        function PopObj = CalObj(obj, PopDec)
            ...
            kt=ceil(obj.Global.D*G);
            mt=0.5+abs(G);
            odd=0;even=0;temp1=0;temp2=0;
            for i = 2:obj.Global.D
                if mod(i,2)
                   temp1=temp1+(PopDec(:,i)-sin(6*pi*PopDec(:,1)+(i+kt)*pi/obj.Global.D)).^2;
                   odd=odd+1;
                else
                  temp2=temp2+(PopDec(:,i)-sin(6*pi*PopDec(:,1)+(i+kt)*pi/obj.Global.D)).^2;
                   even=even+1;  
                end
            end
